...
int compileProgramm(string fileName, string path, string flags, string linkFiles, int granularity){
  ...
  opt = opt.append(programmDir).append("/cache-sim/llvmPass/InsertPass/ProfilePass.so -function");
  stringstream g;
  g << granularity;
  ...
  linkProgramm = linkProgramm.append(programmDir).append("/cache-sim/bin/linkedFunctions/library/profile.o /tmp/profile/output.ll -o /tmp/profile/output");
